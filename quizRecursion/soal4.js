...
function totalDigitRekursif(angka) {
  if (angka < 10) { // Base case
    return angka; 
  } else {
    const digitTerakhir = angka % 10; // 512 % 10 = 2
    const sisaDigit = Math.floor(angka / 10); // 512 / 10 = 51
    return digitTerakhir + totalDigitRekursif(sisaDigit) // 2 + 51 ???????????????????????????????/
  }
}
